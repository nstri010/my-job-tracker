...
if st.session_state['logged_in']:
    ...
    with st.expander("➕ Add New Application", expanded=True):
        ...
        final_desc = st.text_area("Job Description", value=st.session_state['formatted_desc'], height=250)
        ...
        if st.button("🔍 Scan for Match"):
            if final_desc and up_file:
                with st.spinner("Analyzing..."):
                    txt = extract_text_from_upload(up_file)
                    st.session_state['match_data'] = get_ai_match_feedback(final_desc, txt)
        ...
        if st.button("💾 Save Application"):
            if comp and pos:
                with st.spinner("Saving and generating PDF..."):
