...
if st.session_state['logged_in']:
    ...
    with col_logout:
        ...
        # Updated to new width='stretch' to clear logs
    ...
    with st.expander("➕ Add New Application", expanded=False):
        ...
        with col_file:
            ...
            # REMOVED: Resume preview write statement that was cluttering the UI
        with col_date:
        ...
        if st.session_state['match_data']:
            ...
            # Display feedback points clearly
            for point in m.get('feedback', []):
                st.write(f"- {point}")
    ...
    if jobs_list:
        ...
        df['created_at'] = pd.to_datetime(df['created_at']).dt.tz_convert(None).dt.strftime('%m/%d/%Y')

        status_options = ["Active", "Applied", "Interview Scheduled", "Interviewed", "Moving On"]

        edited_df = st.data_editor(
            df,
            width="stretch", # Updated to clear log warnings
            num_rows="dynamic",
            column_config={
                ...
                "pdf_url": st.column_config.LinkColumn("Job Snapshot", display_text="View"),
                "resume_link": st.column_config.LinkColumn("My Resume", display_text="Download"),
                "job_url": st.column_config.LinkColumn("Original Posting Link", display_text="Open Link"),
